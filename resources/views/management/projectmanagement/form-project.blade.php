<form action="{{ url('/project/submitproject') }}" method="POST" id="form_project">
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-12">
            <input type="text" class="form-control" name="title" value="{{ $id ? $data['szProjectName'] : '' }}"
                required {{ $action == 'edit' ? 'readonly' : '' }} autocomplete="off">
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-12">
            <input type="text" class="form-control" name="jira" value="{{ $id ? $data['szJiraProject'] : '' }}"
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-12">
            <input type="text" class="form-control" name="brdtittle" value="{{ $id ? $data['szBRDTittle'] : '' }}"
                required autocomplete="off">
        </div>
    </div>
    <!-- Field Judul Dev -->
    <div class="form-group row">
        ...
        <div class="col-lg-12">
            <input type="text" class="form-control" name="devtittle" value="{{ $id ? $data['szDevTittle'] : '' }}"
                required autocomplete="off">
        </div>
    </div>
                ...
                <div class="col-lg-12">
                    @if ($action == 'detail')
                        <input type="text" class="form-control" name="application"
                            value="{{ $id ? $data['szApplicationId'] : '' }}" required autocomplete="off">
                    @else
                        <select class="select2 " name="application" required>
                            <option value="">-- Select Role --</option>
                            @if ($id)
                                @foreach ($application as $row)
                                    <option value="{{ $row['szApplicationId'] }}"
                                        {{ strtolower($data['szApplicationId']) == strtolower($row['szApplicationId']) ? 'selected' : '' }}>
                                        {{ $row['szApplicationName'] }}</option>
                                @endforeach
                            @else
                                @foreach ($application as $row)
                                    <option value="{{ $row['szApplicationId'] }}">{{ $row['szApplicationName'] }}
                                    </option>
                                @endforeach
                            @endif
                        </select>
